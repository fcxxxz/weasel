...
## 12. 隐藏期跳过 UI 布局：p95 决定性超越（2026-09-01 夜，终版）

### 12.1 逐层定位（每层都有实测）

5 轮交叉复测确认 p95 差距真实（本构建 153-205µs vs 官方 58-105）。
逐层插桩：处理器内部清白（引擎 p95=15µs + 响应 p95=11µs = 26µs）；
worker 分发段 p95=173µs——差额 ~145µs 全部落在 _UpdateUI 的 DWrite
布局量测。面板隐藏期间（inline_preedit 与 TSF 宿主自绘场景）每键跑
全套布局，纯浪费。

### 12.2 修复（剃刀：不做的功才是最快的功）

- UI::Update：面板不可见时只保存（已缩写裁剪的）上下文并置
  dirty_on_show，跳过全部布局
- 显示路径（Show/ShowWithTimeout/RepositionPreview）先补一次 Refresh
  再显示，保证显示帧内容与尺寸最新
- WeaselPanel::MoveTo：隐藏时只记录输入位置，显示时定位

### 12.3 终版对照（同配置交叉 5 轮，真实虚拟键，机器同态）

| 指标 | 本构建 | 官方 master | 相对 |
|---|---:|---:|---|
| 按键 p50 | **30.7–33.1µs** | 44.4–48.9 | 快 ~33% |
| 按键 p95 | **51.8–57.3µs** | 67.4–72.3 | 快 ~24% |
| 按键 p99 | **71.7–75.3µs** | 109.5–126.4 | 快 ~38% |
| 首会话 | **2.0–2.7ms** | 89–246ms | 快 40–90× |
| 会话切换 | **0.8–1.5ms** | 0.9–89ms | |
| 空闲/打字内存 | 62–67MB（GPU 驻留） | 55–68MB | 持平 |
| 内存（WEASEL_WARP=1） | **7–14MB** | — | 低内存偏好可选 |

fxliang pb 同批次：p50 64.7–67.1 / p95 99.9–102（更慢）。

### 12.4 终版墨菲压测（全过）

冷启 15.8s 就绪；3×30k 键（90k）句柄 472 纹丝不动、提交 66.3MB 钉死、
p95 全程 53-57µs；双客户端并发 p95 59.8µs 无死锁；打字中强杀客户端
无损续服；干净退出。

### 12.5 验证完备性声明

- 单测/parser 门全绿；Release x64 全解决方案重建
- 插桩全部剥离（p99 曾被每 200 键日志写盘污染至 330µs，剥离后
  p99=72-75µs，证明终版数字无测量伪影）
- 对照三方均为各自最新源码（官方 d73f629 / fxliang 4e1206f）同配置
  全新部署交叉多轮
