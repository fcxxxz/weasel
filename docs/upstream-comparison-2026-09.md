...
## 9. 三方实机对比（2026-09-01，/realseat 真实管道打字基准）

方法：同机串行；三方共用同一份预置数据与 mohu 用户配置副本（各自全新部署）；
负载 = 300 轮「nihao 」（1800 键）+ 10 会话 churn；真实 IPC 客户端测量。
测试期间用户 TSF 会自动复活旧服务器干扰管道——每阶段前后清扫全部实例
并删除全局 manual-exit 标记（旧版构建的标记不区分命名空间）。

| 指标 | 官方 0.17.4 | fxliang pb | 本构建 |
|---|---|---|---|
| 热启动→可用 (ms) | 2035–2073 | 920–941 | 3289–3627 |
| 首会话 (ms) | 1037 | 569 | 942 |
| 首键 (ms) | 39.0 | 57.6 | 47.0 |
| 按键 p50 (µs) | 5459 | 4485 | 5506 |
| 按键 p95 / p99 (ms) | 149 / 162 | 42 / 66 | 155 / 166 |
| 会话 churn p50 (ms) | >2000（超时） | 286 | 905 |
| 打字后 WS / 私有提交 (MB) | 690 / 206 | 49 / 60 | 95 / 108 |
| 打字后线程 / 句柄 | 49 / 518 | 25 / 401 | 47 / 1710 ⚠ |

结论：对官方版优势明确（打字内存 690→95MB、会话不再 2s 超时、线程不增长）；
但相对 fxliang 全面落后（尾延迟 3.7×、会话成本 3.2×、热启动 3.6×、提交内存 1.8×），
且发现每键句柄累积（471→1710/1800 键）疑似泄漏——三项均为下一步工作。
